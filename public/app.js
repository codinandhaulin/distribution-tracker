...
async function addTicker() {
  ...
  if (!costBasis || costBasis <= 0) {
    toast("Enter a valid cost basis per share.", true);
    ce.focus();
    return;
  }
  if (S.tickers.find((t) => t.symbol === symbol)) {
    toast(`${symbol} is already in your list.`, true);
    se.select();
    return;
  }
...
function removeTicker(symbol) {
  ...
  delete S.loading[symbol];
  delete S.errors[symbol];
  persist();
  render();
}
...
function saveEdit() {
  ...
  if (!cost || cost <= 0) {
    toast("Enter a valid cost basis.", true);
    return;
  }
...
function toggleAddCard(forceOpen) {
  ...
  const open =
    forceOpen !== undefined ? forceOpen : body.classList.contains("collapsed");
...
function toggleView(v) {
  ...
  document
    ...
    .classList.toggle("active", !calView);
...
function renderCalendar() {
  ...
  const addEv = (
    ...
    if (type === "pay") {
      ...
      const ex = events[date].find(
        (e) => e.symbol === symbol && e.type === "pay",
      );
      if (ex) {
  ...
  for (const { symbol, shares } of S.tickers) {
    ...
    for (let i = 0; i < hist.length; i++) {
      const h = hist[i],
        prev = hist[i + 1]; // history is newest-first; prev is older
      if (h.date.startsWith(monthPfx)) addEv(h.date, symbol, "ex");
  ...
  for (const evs of Object.values(events)) {
    for (const ev of evs) {
      if (ev.type === "pay" && ev.amount != null) {
        monthlyTotal += ev.amount;
        hasMonthlyAmt = true;
      }
    }
  }
...
function sortVal(col, t) {
  ...
  switch (col) {
    ...
    case "paydate":
      return d?.dividendDate || null;
    case "dist":
      return d?.distributionAmount ?? null;
    case "freq":
      return d?.frequency ?? null;
    case "annrate":
      return d?.annualDividendRate ?? null;
    case "yop":
      return d?.currentPrice && d?.annualDividendRate
        ? d.annualDividendRate / d.currentPrice
        : null;
    case "yoc":
      return t.costBasis && d?.annualDividendRate
        ? d.annualDividendRate / t.costBasis
        : null;
    case "estpay":
      return t.shares && d?.distributionAmount
        ? t.shares * d.distributionAmount
        : null;
    case "annpayout":
      return t.shares && d?.annualDividendRate
        ? t.shares * d.annualDividendRate
        : null;
    default:
      return null;
  }
...
function sortedTickers() {
  return [...S.tickers].sort((a, b) => {
    const va = sortVal(sortCol, a),
      vb = sortVal(sortCol, b);
...
function renderRow({ symbol, costBasis, shares }) {
  const data = S.data[symbol],
    loading = S.loading[symbol],
    err = S.errors[symbol];
  ...
  if (data.exDividendDate) {
    let pillClass = "pill-past",
      pillText = "past";
    if (upcoming) {
      if (days <= 7) {
        pillClass = "pill-urgent";
        pillText = days === 0 ? "today" : `in ${days}d`;
      } else if (days <= 30) {
        pillClass = "pill-soon";
        pillText = `in ${days}d`;
      } else {
...
function renderSummary() {
  ...
  for (const { symbol, costBasis, shares } of S.tickers) {
    const d = S.data[symbol];
    if (!d) continue;
    const ex = d.exDividendDate
      ? new Date(d.exDividendDate + "T12:00:00Z")
      : null;
    if (ex && ex >= now && ex <= in7) urgent++;
    if (ex && ex >= now && ex <= in30 && shares && d.distributionAmount > 0)
      payout30 += shares * d.distributionAmount;
    const r = d.annualDividendRate || 0;
    if (r > 0 && costBasis) {
      yocSum += (r / costBasis) * 100;
      yocN++;
    }
    if (r > 0 && d.currentPrice) {
      yopSum += (r / d.currentPrice) * 100;
      yopN++;
    }
    if (shares) {
      annualTotal += r * shares;
      hasAnnual = true;
    }
    if (shares != null && d.currentPrice) {
      mktVal += shares * d.currentPrice;
      hasMktVal = true;
    }
    if (shares != null && costBasis) {
  ...
  document.getElementById("s-payout").textContent =
    payout30 > 0 ? fmt$(payout30) : "—";
  document.getElementById("s-yoc").textContent = yocN
    ? fmtPct(yocSum / yocN)
    : "—";
  document.getElementById("s-yop").textContent = yopN
    ? fmtPct(yopSum / yopN)
    : "—";
  document.getElementById("s-annual").textContent = hasAnnual
    ? fmt$(annualTotal)
    : "—";

  document.getElementById("s-mktval").textContent = hasMktVal
    ? fmt$(mktVal)
    : "—";
  document.getElementById("s-cost").textContent = hasCost
    ? fmt$(totalCost)
    : "—";
  ...
  if (hasMktVal && hasCost) {
    ...
    const sign = gain >= 0 ? "+" : "";
    gainEl.textContent =
      fmt$(gain) + (gainPct != null ? ` (${sign}${gainPct.toFixed(2)}%)` : "");
...
function computeMonthlyProjections() {
  ...
  const months = Array.from({ length: 12 }, (_, i) => {
    const t = nowM + i,
      y = nowY + Math.floor(t / 12),
      m = t % 12;
    return { year: y, month: m, total: 0, breakdown: [] };
  });
  const idxMap = {};
  months.forEach((m, i) => {
    idxMap[`${m.year}-${String(m.month + 1).padStart(2, "0")}`] = i;
  });
  ...
  for (const { symbol, shares } of S.tickers) {
    ...
    for (const slot of months) {
      for (const exDate of occurrencesInMonth(
        ...
        if (ex) ex.amount += amt;
        else months[si].breakdown.push({ symbol, amount: amt });
      }
    }
  }
  return months;
...
function renderChart() {
  ...
  document.getElementById("chart-annual-ttl").textContent =
    annTotal > 0 ? fmt$(annTotal) : "";
  const thisYear = now.getFullYear();
  document.getElementById("bar-chart").innerHTML = chartMonths
    .map((m, i) => {
      const h =
        m.total > 0
          ? Math.max(Math.round(((m.total - floor) / range) * BAR_MAX_H), 4)
          : 0;
      const isCur = m.year === thisYear && m.month === now.getMonth();
      const isNextYear = m.year !== thisYear;
      const isBreak =
        isNextYear && (i === 0 || chartMonths[i - 1].year === thisYear);
      const cls = [
        "bar-col",
        isCur && "cur-month",
...
function openSymbolModal(symbol) {
  const d = S.data[symbol];
  const t = S.tickers.find((t) => t.symbol === symbol);
  document.getElementById("sym-modal-title").textContent = d?.name
    ? `${symbol} — ${d.name}`
    : symbol;
  ...
  if (!d) {
    body.innerHTML =
      '<p style="color:var(--muted);padding:16px 18px">No data loaded for this ticker yet.</p>';
    document.getElementById("symbol-modal").classList.remove("hidden");
    return;
  }

  const yoc =
    t?.costBasis && d.annualDividendRate
      ? (d.annualDividendRate / t.costBasis) * 100
      : null;
  const yop =
    d.currentPrice && d.annualDividendRate
      ? (d.annualDividendRate / d.currentPrice) * 100
      : null;
      ...
      <div class="hist-chart">
        ${allBars
          .map((b, i) => {
            const h =
              b.amount > 0
                ? Math.max(Math.round(((b.amount - floor) / range) * BAR_H), 2)
                : 0;
            const dt = parseISO(b.date);
            const lbl =
              i % step === 0 || i === allBars.length - 1
  ...
  for (let i = 0; i < histNewFirst.length; i++) {
    const h = histNewFirst[i],
      prev = histNewFirst[i + 1];
  ...
  if (!rows)
    rows = `<tr><td colspan="5" style="text-align:center;color:var(--muted);padding:20px 18px;font-family:var(--font)">No distribution history available</td></tr>`;
  ...
  body.innerHTML =
    statsHtml + chartHtml + `<div style="padding-top:14px">${tableHtml}</div>`;
  document.getElementById("symbol-modal").classList.remove("hidden");
}
    ...
    .map((ev) => {
      ...
      const amt =
        isPay && ev.amount
          ? fmt$(ev.amount)
          : ev.perShare
            ? `${fmt$(ev.perShare, 4)}/sh`
            : "";
      const estBadge = ev.est
        ? `<span class="day-modal-est" style="color:${typeColor}">est</span>`
        : "";
...
function render() {
  ...
  if (calView) renderCalendar();
    ...
    renderHeaders();
    document.getElementById("tbody").innerHTML = sortedTickers()
      .map(renderRow)
      .join("");
  }
}
...
async function handleCSVFile(input) {
  ...
  if (headerIdx < 0) {
    toast('No "Ticker" or "Symbol" column found — check CSV format.', true);
    return;
  }
  ...
  const costPSIdx = h.findIndex((c) =>
    /^average cost basis$|\/share|per share|avg(erage)?\s+cost(?!\s+(basis\s+)?total)/i.test(
      c,
    ),
  );
  const costTotIdx =
    costPSIdx < 0
      ? h.findIndex((c) =>
          /cost basis total|total cost basis|total cost|^cost basis$/i.test(c),
        )
      : -1;
  ...
  for (let i = headerIdx + 1; i < lines.length; i++) {
    ...
    if (
      !symbol ||
      symbol.length > 12 ||
      /^(--|cash|pending|total|account)/i.test(raw)
    )
      continue;
  ...
  if (!imported.length) {
    toast("No valid positions found — check CSV format.", true);
    return;
  }
  ...
  if (replaceAll) {
    ...
    render();
    toast(
      `Replaced portfolio with ${imported.length} ticker${imported.length !== 1 ? "s" : ""} — fetching data…`,
    );
    await batchFetch(S.tickers);
  } else {
    ...
    toast(
      `CSV import: ${parts.join(", ")}${newSymbols.length ? " — fetching new tickers…" : ""}`,
    );
    if (newSymbols.length)
      await batchFetch(newSymbols.map((s) => ({ symbol: s })));
...
async function startApp() {
  ...
  document.getElementById("bar-chart").addEventListener("mouseover", (e) => {
    ...
    const rows = [...m.breakdown]
      ...
      .map(
        (b) =>
          `<div class="tip-row"><span class="tip-sym">${b.symbol}</span><span class="tip-amt">${fmt$(b.amount)}</span></div>`,
      )
      .join("");
  ...
  try {
    ...
    document
      .getElementById("setup-banner")
      .classList.toggle("hidden", cfg.hasKey);
  } catch {}
