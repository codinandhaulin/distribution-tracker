...
- **Request coalescing** (`polyInFlight` map in `server.js`): concurrent non-force requests for the same symbol share one queue entry/promise instead of each enqueuing separately. Without this, the page-load `Promise.all` (fires all tickers in parallel) and an overlapping Refresh All/Hard Refresh (sequential `batchFetch`) would each queue their own copy of every ticker — the second wave sits behind the first, so a tab's own "(i/N)" progress counter can look stuck long after the server has moved on to different tickers from the other wave. Coalescing means only one real fetch happens per symbol regardless of how many callers ask for it concurrently
