...
const JWT_SECRET = process.env.JWT_SECRET || (() => {
  console.warn('  ⚠  JWT_SECRET not set — sessions will reset on restart. Add JWT_SECRET to .env');
...
function getAuthUser(req) {
  ...
  if (!m) return null;
  try { return jwt.verify(decodeURIComponent(m[1]), JWT_SECRET); } catch { return null; }
}
...
async function polyGet(endpoint) {
  ...
  if (!key) { const e = new Error('POLYGON_KEY not configured'); e.noKey = true; throw e; }
  const sep = endpoint.includes('?') ? '&' : '?';
  const res = await fetch(`${POLY_BASE}${endpoint}${sep}apiKey=${key}`, {
    headers: { Accept: 'application/json', 'User-Agent': 'DistributionTracker/2.0' }
  });
...
async function polyGetRetry(endpoint) {
  try { return await polyGet(endpoint); }
  catch (err) {
...
let polyLastAt = 0, polyRunning = false;
  ...
  try {
    while (polyQueue.length > 0) {
      ...
      if (!item.force) {
        ...
        if (cached) { polyQueue.shift(); item.resolve(cached); continue; }
      }
      const wait = POLY_INTERVAL - (Date.now() - polyLastAt);
      if (polyLastAt > 0 && wait > 0) {
        console.log(`  [queue] ${polyQueue.length} pending — waiting ${Math.round(wait/1000)}s`);
...
app.post('/api/login', async (req, res) => {
  ...
  res.setHeader('Set-Cookie', `dt_tok=${encodeURIComponent(token)}; HttpOnly; Path=/; SameSite=Strict; Max-Age=${7*24*3600}`);
  res.json({ username });
});
